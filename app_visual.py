...
try:
    import fisica_ambiental
except ImportError:
    st.error("⚠️ Falta el archivo 'fisica_ambiental.py' en el directorio.")
...
with st.sidebar:
    ...
    st.markdown("### `v12.0 // FULL SUITE`") 
    selected_city = st.selectbox("📍 UBICACIÓN", list(LOCATIONS.keys()), key="city_sel_final")
    st.divider()
    map_layer = st.radio("Vista Mapa:", ["Táctico", "Satélite"])
    st.divider()
    refresh_rate = st.slider("Refresh (s):", 60, 300, 60, key="refresh_slider_final")
...
# TABS COMPLETOS
tab1, tab2, tab3, tab4 = st.tabs(["📡 MONITOREO", "📈 ANALÍTICA", "🔮 PRONÓSTICO", "🧪 SIMULADOR"])
...
with tab1:
    if model and temp:
        ...
        with k3: st.markdown(f"""<div class="css-card"><div class="metric-label">VIENTO</div><div class="big-metric">{wind_s} m/s</div><div style="color:#888;font-size:12px">{deg_to_cardinal(wind_d)}</div></div>""", unsafe_allow_html=True)
        ...
        for city, coords in LOCATIONS.items():
            ...
            if ct:
                ...
                popup_content = f"""<div style="font-family:sans-serif"><b>{city}</b><br>Temp: {ct}°C<br>AQI: {caqi}<br>Viento: {deg_to_cardinal(wind_d)}</div>"""
        ...
        st.error("⚠️ Error de Conexión: Límite de API excedido o red caída.")
